function [H0,H0ij] = poly_product_DWR(W0,W0ij,R0_cell,R0ij_cell,numvar,corder,max_order)  
% Comment: the 0th order can be computed with this function 
nvar = numvar;
H_sub = zeros(repmat(max_order+1,1,nvar));
...
if corder == 0     
    for m = 1:nvar
        ...
        R0 = R0_cell{m};
        eval(strcat('H_sub(',ui_cell,') = H_sub(',ui_cell,')+W0(',ei_cell,')*R0(',ui_cell,');'))
    end
else
    order = corder;  
        ...
        for i=1:size(nonz_kcoef,1)
            ...
            if  m(i_min)==0 
                ...
                if sum(m<=k+ej)==nvar
                    ...
                    eval(strcat('Hadd = m(i_min)*W0(',m_cell,')*R0(',k_m_cell,');'));
                    eval(strcat('H_sub(',k_cell,') = H_sub(',k_cell,')+Hadd;'));
                end
            end
        end
    end
...
xi = find(H_sub);
Xi_cell = cell(1,nvar);
[Xi_cell{:}] = ind2sub(size(H_sub),xi);
Xi = [Xi_cell{:}];
H0ij = Xi-ones(size(Xi));
