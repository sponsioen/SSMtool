function [H0,H0ij] = poly_product_DW0R1(W0,W0ij,R0_cell,R0ij_cell,numvar,corder,max_order,symbolic)  
...
if corder == 0     
    for m = 1:nvar
        for j=1:2
            ...
            R0 = R0_cell{m};
            eval(strcat('H_sub(',ui_cell,',',num2str(j),') = H_sub(',ui_cell,',',num2str(j),')+W0(',ei_cell,')*R0(',ui_cell,',',num2str(j),');'))
        end
    end
else
    order = corder;  
        ...
        for i=1:size(nonz_kcoef,1)
            ...
            if  m(i_min)==0 
                ...
                if sum(m<=k+ej)==nvar
                    ...
                    for l=1:2
                        eval(strcat('Hadd = m(i_min)*W0(',m_cell,')*R0(',k_m_cell,',',num2str(l),');'));
